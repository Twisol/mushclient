...
BOOL CColourPickerDlg::OnInitDialog() 
{
  ...
  for (ColoursIterator itr = App.m_ColoursMap.begin(); itr != App.m_ColoursMap.end(); ++itr, ++iItem)
    // remember RGB code
    if (m_ctlColourList.InsertItem (iItem, itr->first) != -1)
      {
  	  m_ctlColourList.SetItemText(iItem, 1, ""); // swatch
      m_ctlColourList.SetItemData (iItem, (unsigned long) itr->second);
      }
...
static COLORREF CustomColours [16] = { 0 };

void CColourPickerDlg::OnSelectColour() 
{
  CColorDialog dlg (
      m_ctlSwatch.m_colour,
...
void CColourPickerDlg::ShowName ()
{
  ...
  m_strMXPname = CFormat ("#%02X%02X%02X", iRed, iGreen, iBlue);

  m_ctlMXPname.SetWindowText     (m_strMXPname);
  m_ctlVBname.SetWindowText      (CFormat ("&&h%02X%02X%02X", iBlue, iGreen, iRed));
  m_ctlJscriptName.SetWindowText (CFormat ("0x%02X%02X%02X", iBlue, iGreen, iRed));
...
void CColourPickerDlg::SelectCurrentColour()
{
...
void CColourPickerDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
  ...
  m_ctlSwatch.m_colour = RGB (
      m_ctlRedSlider.GetPos (),
