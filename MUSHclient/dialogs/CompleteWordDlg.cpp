...
CCompleteWordDlg::CCompleteWordDlg(CWnd* pParent /*=NULL*/)
  : CDialog(CCompleteWordDlg::IDD, pParent),
    m_bLua(false), m_bFunctions(true), m_extraItems(NULL)
{}
...
void CCompleteWordDlg::ReloadList ()
{
  ...
  if (m_bFunctions)
    {
    for (int i = 0; sFunctions [i] [0] != NULL; ++i)
      {
    ...
    if (m_bLua)
      ...
      for (it = LuaFunctionsSet.begin (); it != LuaFunctionsSet.end (); ++it)
        {
      ...
      for (it = LuaTablesSet.begin (); it != LuaTablesSet.end (); ++it)
        {
        strFunction = it->c_str ();
        strFunction = strFunction.Left (iFilterLength);
        strFunction.MakeLower ();
  ...
  if (m_extraItems)
    ...
    for (it = m_extraItems->begin (); it != m_extraItems->end (); ++it)
      {
      strFunction = it->c_str ();
      strFunction = strFunction.Left (iFilterLength);
      strFunction.MakeLower ();
