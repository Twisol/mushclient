...
BOOL CCreditsDlg::OnInitDialog() 
{
  ...
  HRSRC hRsrc = FindResource (App.m_hInstDLL, MAKEINTRESOURCE (m_iResourceID), "TEXT");
  ...
  if (hRsrc)
    {
    HGLOBAL hRsrc_text = LoadResource (App.m_hInstDLL, hRsrc);
  ...
  m_ctlCreditsText.SendMessage (WM_SETFONT, (WPARAM) m_font.m_hObject, MAKELPARAM (TRUE, 0));
