...
#define ADJUST_BUTTON(ctl, item) \
  ctl.MoveWindow (iBorder + (iWidth * (item - 1)) + (iGap * (item - 1)), iTopOfRow, iWidth, iHeight)
...
void CEditMultiLine::OnGotoLine() 
{
  ...
  dlg.m_iLineNumber = m_ctlText.LineFromChar () + 1; // default to the current line number
