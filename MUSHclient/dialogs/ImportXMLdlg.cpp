...
void CImportXMLdlg::OnOk() 
{
  ...
  try
    ...
    {
    ::UMessageBox (
        TFormat ("Unable to open or read %s", (LPCTSTR) filedlg.GetPathName ()),
        MB_ICONEXCLAMATION
        );
    e->Delete ();
...
void CImportXMLdlg::ImportArchive (CArchive & ar)
{
  ...
  try
    ...
    if (m_ctlGeneral.GetCheck ())   iMask |= XML_GENERAL;
    if (m_ctlTriggers.GetCheck ())  iMask |= XML_TRIGGERS;
    if (m_ctlAliases.GetCheck ())   iMask |= XML_ALIASES;
    if (m_ctlTimers.GetCheck ())    iMask |= XML_TIMERS;
    if (m_ctlMacros.GetCheck ())    iMask |= XML_MACROS;
    if (m_ctlVariables.GetCheck ()) iMask |= XML_VARIABLES;
    if (m_ctlColours.GetCheck ())   iMask |= XML_COLOURS;
    if (m_ctlKeypad.GetCheck ())    iMask |= XML_KEYPAD;
    if (m_ctlPrinting.GetCheck ())  iMask |= XML_PRINTING;

    UINT iTriggers = 0, iAliases   = 0, iTimers  = 0,
         iMacros   = 0, iVariables = 0, iColours = 0,
         iKeypad   = 0, iPrinting  = 0;

    // do it
    m_pDoc->Load_World_XML (
        ar,
        iMask | XML_NO_PLUGINS,
        0,          // load flags
        &iTriggers, &iAliases,   &iTimers,
        &iMacros,   &iVariables, &iColours,
        &iKeypad,   &iPrinting
        );
    ...
    {
    ::TMessageBox ("There was a problem parsing the XML. See the error window for more details");
    e->Delete ();
    }
