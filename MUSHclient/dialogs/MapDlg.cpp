...
CMapDlg::CMapDlg(CWnd* pParent /*=NULL*/)
  : CDialog(CMapDlg::IDD, pParent),
    m_pDoc(NULL), m_font(NULL), m_iItemCount(0)
{
...
void CMapDlg::DoDataExchange(CDataExchange* pDX)
{
  ...
  if (pDX->m_bSaveAndValidate)
    ...
    if (m_bMapFailureRegexp && !CheckRegularExpression (m_strMappingFailure, 0))
      {
      DDX_Text(pDX, IDC_MAPPING_FAILURE, m_strMappingFailure);
      pDX->Fail();
...
void CMapDlg::OnRemoveAll() 
{
  if (m_pDoc->m_strMapList.GetCount () > 1)
    if (UMessageBox (
          TFormat ("Remove existing %i directions from the map?",m_pDoc->m_strMapList.GetCount ()),
...
void CMapDlg::CalculateSpeedWalks ()
{
...
{
  m_pDoc->SendMsg (
      m_pDoc->DoEvaluateSpeedwalk (GetText (m_ctlBackwards)),
