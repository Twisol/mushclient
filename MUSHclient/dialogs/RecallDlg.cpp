...
CRecallDlg::CRecallDlg(COLORREF back_colour, CWnd* pParent /*=NULL*/)
  : CDialog(CRecallDlg::IDD, pParent),
    m_first_time(true), m_font(NULL), m_bReadOnly(TRUE),
...
void CRecallDlg::OnOK ()
{
  if (m_ctlText.GetModify ())
    {
    int iReply = ::TMessageBox (
        "Window contents have changed. Save changes?",
        MB_ICONQUESTION | MB_YESNOCANCEL
        );

    if (iReply == IDCANCEL ||
        iReply == IDYES && !SaveModified (false)) // must have cancelled save
      return;
    }
