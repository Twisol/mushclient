...
CTextAttributesDlg::CTextAttributesDlg(CWnd* pParent /*=NULL*/)
  : CDialog(CTextAttributesDlg::IDD, pParent),
    m_pDoc(NULL), m_pLine(NULL), m_iLine(0)
{
...
static void ShowInfo (CString arg, CMUSHclientDoc * pDoc)
{
  pDoc->AppendToTheNotepad (
    "Line Information - " + pDoc->m_mush_name,
    arg + ENDLINE,
    false,
    eNotepadLineInfo
    );
...
void CTextAttributesDlg::OnLineInfo() 
{
  ...
  CString strDate = m_pLine->m_theTime.Format ("%A, %B %d, %#I:%M:%S %p");
  ...
  CString strText(m_pLine->text, m_pLine->len);
  ...
  POSITION pos;
  for (pos = m_pLine->styleList.GetHeadPosition(); pos != NULL; ++iCount)
    {
    ...
    switch (pStyle->iFlags & ACTIONTYPE)
      ...
      case ACTION_NONE: 
        ...
        break;
      case ACTION_SEND: 
        INFO (TFormat (" Action - send to MUD: \"%s\"", (LPCTSTR) strAction));
        if (!strHint.IsEmpty ())
          INFO (TFormat (" Hint: \"%s\"", (LPCTSTR) strHint));
        break;
      case ACTION_HYPERLINK: 
        INFO (TFormat (" Action - hyperlink: \"%s\"", (LPCTSTR) strAction));
        if (!strHint.IsEmpty ())
          INFO (TFormat (" Hint: \"%s\"", (LPCTSTR) strHint));
        break;
      case ACTION_PROMPT: 
        INFO (TFormat (" Action - send to command window: \"%s\"", (LPCTSTR) strAction));
        if (!strHint.IsEmpty ())
          INFO (TFormat (" Hint: \"%s\"", (LPCTSTR) strHint));
        break;
      } // end of switch
    ...
    if (pStyle->iFlags & START_TAG)
      INFO (TFormat (" Start MXP tag: %s", (LPCTSTR) pStyle->pAction->m_strAction));

    static const char * sColours [8] = {
      "Black",
      "Red",  "Green",   "Yellow",
      "Blue", "Magenta", "Cyan",
      "White",
  ...
  INFO (TFormat (
      ...
      PLURAL (iCol),
      PLURAL (m_pLine->styleList.GetCount ())
      ));
  if (iCol != m_pLine->len)
