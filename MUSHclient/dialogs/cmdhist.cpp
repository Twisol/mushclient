...
void CCmdHistory::DoDataExchange(CDataExchange* pDX)
{
  ...
  if(!(pDX->m_bSaveAndValidate))
    ...
    while (pos)
      ...
      int nItem = pList->AddString(str);  // add to list (truncated to 504 chars)
...
void CCmdHistory::OnOK() 
{
  ...
  CString str = m_msgList->GetAt (pos);
...
void CCmdHistory::DoFind (bool bAgain)
{
  ...
  bool found = FindRoutine (
      m_msgList,           // passed back to callback routines
      *m_pHistoryFindInfo, // finding structure
      InitiateSearch,      // how to re-initiate a find
      GetNextLine          // get the next line
      );

  pList->SetCurSel((found) ? m_pHistoryFindInfo->m_nCurrentLine : -1);
  OnSelchangeCommands ();
} // end of CCmdHistory::DoFind


void CCmdHistory::InitiateSearch (const CObject * pObject, CFindInfo & FindInfo)
{
...
bool CCmdHistory::GetNextLine (const CObject * pObject, CFindInfo & FindInfo, CString & strLine)
{
  CStringList* pMsgList = (CStringList*) pObject;
...
void CCmdHistory::OnNotepad() 
{
  ...
  CreateTextWindow (
      ...
      m_pDoc->m_input_font_name,   m_pDoc->m_input_font_height,
      m_pDoc->m_input_font_weight, m_pDoc->m_input_font_charset,
      m_pDoc->m_input_text_colour, m_pDoc->m_input_background_colour,
      "",       // search string
      "",       // line preamble
      false, false, false,
      false, false, false,
