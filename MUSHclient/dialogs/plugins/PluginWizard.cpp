...
CPluginWizardPage1::CPluginWizardPage1()
  : CPropertyPage(CPluginWizardPage1::IDD)
{
...
CPluginWizardPage2::CPluginWizardPage2()
  : CPropertyPage(CPluginWizardPage2::IDD)
{
...
CPluginWizardPage3::CPluginWizardPage3()
  : CPropertyPage(CPluginWizardPage3::IDD)
{
...
BOOL CPluginWizardPage3::OnInitDialog() 
{
  ...
  int iItem = 0;
  for (int iTrigger = 0; iTrigger < m_doc->m_TriggerArray.GetSize (); ++iTrigger)
    {
...
void CPluginWizardPage3::OnOK() 
{
  ...
  for (int i = 0; i < m_doc->m_TriggerArray.GetSize (); ++i)
    m_doc->m_TriggerArray [i]->bSelected = false;

  for (int nItem = -1; (nItem = m_ctlList.GetNextItem(nItem, LVNI_SELECTED)) != -1;)
    {
...
CPluginWizardPage4::CPluginWizardPage4()
  : CPropertyPage(CPluginWizardPage4::IDD)
{
...
BOOL CPluginWizardPage4::OnInitDialog() 
{
  ...
  int iItem = 0;
  for (int iAlias = 0; iAlias < m_doc->m_AliasArray.GetSize (); ++iAlias)
    {
...
{
  // de-select everything
  for (int i = 0; i < m_doc->m_AliasArray.GetSize (); ++i)
...
BOOL CPluginWizardPage5::OnInitDialog() 
{
  ...
  for ( POSITION pos = m_doc->m_TimerMap.GetStartPosition(); pos;)
    {
    CString strName;
    CTimer * t = NULL;
    ...
    if (m_ctlList.InsertItem (iItem, t->strLabel) != -1)
      ...
      if (t->iType == CTimer::eInterval)
        strTimer = TFormat (
            "Every %02i:%02i:%04.2f", 
            t->iEveryHour, t->iEveryMinute, t->fEverySecond
            );
      else
        strTimer = TFormat (
            "At %02i:%02i:%04.2f",
            t->iAtHour, t->iAtMinute, t->fAtSecond
            );
...
CPluginWizardPage6::CPluginWizardPage6()
  : CPropertyPage(CPluginWizardPage6::IDD)
{
...
BOOL CPluginWizardPage6::OnInitDialog() 
{
  ...
  int iItem = 0;
  for (POSITION pos = m_doc->m_VariableMap.GetStartPosition(); pos != NULL;)
    {
    CString strName;
    CVariable * v = NULL;
...
void CPluginWizardPage6::OnOK() 
{
  ...
  for (int nItem = -1; (nItem = m_ctlList.GetNextItem(nItem, LVNI_SELECTED)) != -1;)
    ...
    for (POSITION pos = m_doc->m_VariableMap.GetStartPosition(); pos;)
      {
      CString strName;
      CVariable * v2 = NULL;
...
CPluginWizardPage7::CPluginWizardPage7()
  : CPropertyPage(CPluginWizardPage7::IDD)
{
...
CPluginWizardPage8::CPluginWizardPage8()
  : CPropertyPage(CPluginWizardPage8::IDD)
{
