...
CTimerDlg::CTimerDlg(CWnd* pParent /*=NULL*/)
  : CDialog(CTimerDlg::IDD, pParent), m_pDoc(NULL),
    m_current_timer(NULL), m_pTimerMap(NULL)
{
...
void CTimerDlg::DoDataExchange(CDataExchange* pDX)
{
  ...
  if (pDX->m_bSaveAndValidate)
    ...
    for (POSITION pos = m_pTimerMap->GetStartPosition (); pos;)
      {
      ...
      if (!m_strLabel.IsEmpty () && // we can have duplicate blank names
          m_strLabel.CompareNoCase (timer_item->strLabel) == 0)
        {
    ...
    if (m_iSendTo == eSendToSpeedwalk)
      ...
      if (!strResult.IsEmpty () && strResult [0] == '*') // error in speedwalk string?
        {
...
void CTimerDlg::OnUpdateInvocationCount(CCmdUI* pCmdUI)
{
  ...
  CString strMessage;
  strMessage.Format ("%ld call%s.", PLURAL(m_current_timer->nInvocationCount));
...
void CTimerDlg::OnUpdateMatchCount(CCmdUI* pCmdUI)
{
  ...
  CString strMessage;
  strMessage.Format ("Fired %ld time%s.", PLURAL(m_current_timer->nMatched));
  pCmdUI->SetText (strMessage);
