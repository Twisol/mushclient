...
CAliasDlg::CAliasDlg(CWnd* pParent /*=NULL*/)
  : CDialog(CAliasDlg::IDD, pParent), m_pDoc(NULL),
    m_pAliasMap(NULL), m_current_alias(NULL), m_font(NULL), m_font2(NULL)
{
...
void CAliasDlg::DoDataExchange(CDataExchange* pDX)
{
  ...
  if (pDX->m_bSaveAndValidate)
    ...
    for (POSITION pos = m_pAliasMap->GetStartPosition (); pos;)
      {
      ...
      if (!m_strLabel.IsEmpty () && // we can have duplicate blank names
          m_strLabel.CompareNoCase (alias_item->strLabel) == 0)
        {
        ::UMessageBox(TFormat (
    ...
    else if (CheckLabel (m_strVariable)) // check variable name is valid
      {
      ::TMessageBox(
    ...
    if ((!m_bExpandVariables || m_contents.Find ('@') == -1) &&
        m_iSendTo == eSendToSpeedwalk && m_contents.Find ('%') == -1)
      {
      CString strResult = m_pDoc->DoEvaluateSpeedwalk (m_contents);

      // error in speedwalk string?
      if (!strResult.IsEmpty () && strResult[0] == '*')
        {
    ...
    if (!m_strProcedure.IsEmpty () &&      // blank procedure is OK
        CheckLabel (m_strProcedure, true)) // check procedure is valid
      {
      ::TMessageBox(
...
void CAliasDlg::OnUpdateInvocationCount(CCmdUI* pCmdUI)
{
  ...
  CString strMessage;
  strMessage.Format ("%ld call%s.", PLURAL(m_current_alias->nInvocationCount));
...
{
  CString strContents = m_pDoc->DoReverseSpeedwalk (GetText (m_ctlAliasContents));

  if (!strContents.IsEmpty () && strContents [0] == '*') // error in speedwalk string?
    {
