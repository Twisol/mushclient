...
{  
  ...
  for (int i = 0; i < NUMITEMS (m_macros); ++i)
    {
...
void CMUSHclientDoc::LoadPrefsP15 (CPrefsP15 &page15)  
{
  ...
  page15.m_strBufferLines.Format (
      "%i / %ld", 
      m_LineList.GetCount (),
      m_maxlines
      );
  page15.m_strTriggers.Format (
      "%i   (%I64d matched)", 
  ...
  if (App.m_iCounterFrequency > 0)
    {
    page15.m_strTriggerTimeTaken.Format (
        "%12.6f seconds.",
        ((double) iTimeTaken) / ((double) App.m_iCounterFrequency)
        );

    if (m_iCompressionTimeTaken != 0)
      page15.m_strTimeTakenCompressing.Format (
          "%12.6f seconds ",
          ((double) m_iCompressionTimeTaken) / ((double) App.m_iCounterFrequency)
          );
    }

  page15.m_strTimeTakenCompressing += (m_bCompress) ? "(MCCP active)" : "(MCCP not active)";

  page15.m_strAliases.Format (
      "%i   (%I64d used)", 
      m_AliasMap.GetCount (),
      nTotalAliases
      );
  page15.m_strTimers.Format (
      "%i   (%I64d fired)",
      m_TimerMap.GetCount (),
      nTotalTimers
      );
  ...
  if (m_nTotalUncompressed)
    {
    page15.m_strCompressionRatio.Format (
        "%6.1f%% (lower is better)",
        (double) m_nTotalCompressed / (double) m_nTotalUncompressed * 100.0
        );
    if (m_iMCCP_type == 1)
  ...
  page15.m_strBytesReceived = CFormat (
      "%I64d bytes (%I64d Kb)",
      m_nBytesIn,
      m_nBytesIn  / (__int64) 1024
      );
  page15.m_strBytesSent = CFormat (
    "%I64d bytes (%I64d Kb)",
    m_nBytesOut,
    m_nBytesOut / (__int64) 1024
    );
...
{  
  ...
  page17.m_strIsActive = (GetScriptEngine ()) ? "(active)" : "(not active)";
  ...
  if (App.m_iCounterFrequency > 0)
    page17.m_strExecutionTime.Format (
        "Time spent: %16.6f seconds.",
        ((double) m_iScriptTimeTaken) / ((double) App.m_iCounterFrequency)
        );
  else
...
void CMUSHclientDoc::LoadPrefsP23 (CPrefsP23 &page23)
{
  ...
  if (m_strOurChatName.IsEmpty ())
    m_strOurChatName = (m_name.IsEmpty ()) ? DEFAULT_CHAT_NAME : m_name;
...
{  
  ...
  return
    bChanged ||
    memcmp (m_customtext, page3.m_customtext, sizeof (m_customtext)) != 0 ||
    memcmp (m_customback, page3.m_customback, sizeof (m_customback)) != 0
    ;
...
bool CMUSHclientDoc::ChangedPrefsP5 (CPrefsP5 &page5)  
{
  ...
  return
    memcmp (m_normalcolour, page5.m_normalcolour, sizeof (m_normalcolour)) != 0 ||
    memcmp (m_boldcolour,   page5.m_boldcolour,   sizeof (m_boldcolour))   != 0 ||
...
bool CMUSHclientDoc:: ChangedPrefsP20 (CPrefsP20 &page20)  
{
  ...
  for (int i = 0; i < 8; ++i)
    ...
    // bold styles
    if (nBold [i])     nNormalPrintStyle [i] |= FONT_BOLD;
    if (nBold [i + 8]) nBoldPrintStyle [i]   |= FONT_BOLD;

    // underline styles
    if (nUnderline [i])     nNormalPrintStyle [i] |= FONT_UNDERLINE;
    if (nUnderline [i + 8]) nBoldPrintStyle [i]   |= FONT_UNDERLINE;

    // italic styles
    if (nItalic [i])     nNormalPrintStyle [i] |= FONT_ITALIC;
    if (nItalic [i + 8]) nBoldPrintStyle [i]   |= FONT_ITALIC;

    if (nNormalPrintStyle [i] != m_nNormalPrintStyle [i] ||
        nBoldPrintStyle [i] != m_nBoldPrintStyle [i])
      bStylesChanged = true;
...
void CMUSHclientDoc::SavePrefsP14 (CPrefsP14 &page14)  
{
  ...
  if (m_maxlines != page14.m_nLines &&
      FixUpOutputBuffer (page14.m_nLines))  // if we could adjust it
    m_maxlines = page14.m_nLines;
...
void CMUSHclientDoc::SavePrefsP17 (CPrefsP17 &page17)
{
  ...
  // b) The script file name has changed; or
  // c) The user has disabled scripting
...
bool CMUSHclientDoc::GamePreferences (const int iPage)
{
  ...
  CTreePropertySheet sheet (150, AppendMUDName ("Configuration"), NULL, iWantedPage);
