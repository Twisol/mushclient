...
CGenPropertyPage::CGenPropertyPage(const UINT nID)
  : CPropertyPage(nID), m_last_col(0), m_reverse(FALSE),
    m_iColWidth(NULL), m_iColJust(NULL), m_iColumnCount(0),
...
void CGenPropertyPage::OnAddItem(CDialog & dlg) 
{
  ...
  add_item (pItem, new CString (strObjectName), 0, TRUE);
  ...
  if (m_doc->m_ScriptEngine)
    {
    CString strMessage;
    SetDispatchID (pItem, m_doc->GetProcedureDispid (
        GetScriptName (pItem),
        m_strObjectType,
        GetLabel (pItem),
        strMessage)
...
void CGenPropertyPage::OnDeleteItem() 
{
  ...
  for (int i = iCount - 1; i >= 0; --i)
    ...
    if (CheckIfIncluded (pItem))
      {
      iIncluded += 1; // don't do message here in case hundreds of them
      continue;
      }

    if (CheckIfExecuting (pItem))
      {
      iExecuting += 1; // don't do message here in case hundreds of them
      continue;
      }
    ...
    m_ObjectMap->RemoveKey (*pstrObjectName); // delete from the map
    delete pstrObjectName;                    // delete its item string
    delete pItem;                             // delete the item itself
    m_ctlList->DeleteItem (nItem);            // and remove from the dialog list control
  } // end of dealing with each selected item
                                            ...
                                            LPARAM lParamSort)
{
  ...
  if (psort_param->pObjectMap->Lookup(*pName1, item1) == 0 ||
      psort_param->pObjectMap->Lookup(*pName2, item2) == 0)
    return 0;
...
void CGenPropertyPage::LoadList ()
{
  long iCount = 0;
  long iNotShown = 0;
   ...
   for (POSITION pos = m_ObjectMap->GetStartPosition (); pos;)
     ...
     if (bFiltering)
       ...
       // use result if we get the exact type: boolean (true/false)
         ...
         if (!bUse)
           iNotShown += 1;  // this one not shown
         }
...
void CGenPropertyPage::DoFind (bool bAgain)
{
  ...
  bool found = FindRoutine (
      m_ctlList,          // passed back to callback routines
...
BOOL CGenPropertyPage::OnInitDialog() 
{
  ...
  App.RestoreColumnConfiguration (
      strDescription,
      m_iColumnCount, *m_ctlList,
      iColOrder, m_iColWidth,
...
void CGenPropertyPage::OnPasteItem() 
{
  ...
  try
    ...
    {
    ::TMessageBox (
        "There was a problem parsing the XML on the clipboard. "
