...
int __stdcall BrowseCallbackProc(HWND hwnd, UINT uMsg,
...
CPrefsP1::CPrefsP1()
  : CPropertyPage(CPrefsP1::IDD),
    m_page_number(1), m_doc(NULL)
{
...
{
  pCmdUI->Enable (
      m_doc->m_sockAddr.sin_addr.s_addr != 0 ||
...
CPrefsP3::CPrefsP3()
  : CPropertyPage(CPrefsP3::IDD),
    m_page_number(1), m_RH_click_count(0),
    m_doc(NULL), m_prefsp5(NULL)
{
...
{
  pCmdUI->Enable (
      m_prefsp5->m_ctlUseDefaultColours.GetCheck () == 0 ||
      App.m_strDefaultColoursFile.IsEmpty ()
      );
...
void CPrefsP3::OnRandom() 
{
  ...
  for (int i = 0; i < MAX_CUSTOM; ++i)
    {
    m_customtext [i]  = (COLORREF) (genrand () * (double) 0x1000000);
    m_customback [i]  = (COLORREF) (genrand () * (double) 0x1000000);
    }
...
CPrefsP5::CPrefsP5()
  : CPropertyPage(CPrefsP5::IDD),
    m_page_number(1), m_doc(NULL), m_prefsp3(NULL)
{
...
void CPrefsP5::DoDataExchange(CDataExchange* pDX)
{
  ...
  if (pDX->m_bSaveAndValidate &&
      m_ctlUseDefaultColours.GetCheck () &&
      m_ctlUseDefaultColours.GetCheck () != m_bUseDefaultColours &&
      ::TMessageBox (
          "By checking the option \"Override with default colours\" "
...
{
  pCmdUI->Enable (
      m_ctlUseDefaultColours.GetCheck () == 0 ||
      App.m_strDefaultColoursFile.IsEmpty ()
      );
...
void CPrefsP5::OnRandom() 
{
  ...
  for (int i = BLACK; i <= WHITE; ++i)
    {
    m_normalcolour [i] = (COLORREF) (genrand () * (double) 0x1000000);
    m_boldcolour [i]   = (COLORREF) (genrand () * (double) 0x1000000);
    }
...
CPrefsP6::CPrefsP6()
  : CPropertyPage(CPrefsP6::IDD),
    m_page_number(1), m_pMacrosFindInfo(NULL), m_doc(NULL),
...
void CPrefsP6::DoDataExchange(CDataExchange* pDX)
{
  ...
  if (pDX->m_bSaveAndValidate)
    if (m_ctlUseDefaultMacros.GetCheck () &&
        m_ctlUseDefaultMacros.GetCheck () != m_bUseDefaultMacros &&
        ::TMessageBox (
            "By checking the option \"Override with default macros\" "
                                    ...
                                    LPARAM lParamSort)
{
  ...
  switch (psort_param->sortkey)   // which sort key
    {
    case eColumnName:
      iResult = strMacroDescriptions [lParam1].CompareNoCase (strMacroDescriptions [lParam2]);
      break;
    case eColumnText:
      iResult = psort_param->dlg->m_macros [lParam1].CompareNoCase (psort_param->dlg->m_macros [lParam2]);
      break;
    case eColumnAction:
      iResult = psort_param->dlg->m_macro_type_alpha [lParam1].CompareNoCase (psort_param->dlg->m_macro_type_alpha [lParam2]);
      break;
...
void CPrefsP6::OnLoadMacro() 
{
  ...
  for (int i = 0; i < NUMITEMS (m_doc->m_macros); ++i)
    {
...
BOOL CPrefsP6::OnInitDialog() 
{
  ...
  int iColWidth [eColumnCount] = {100, 180, 0};
...
{
  pCmdUI->Enable (
    m_ctlMacroList.GetSelectedCount () != 0 &&
...
void CPrefsP6::DoFind (bool bAgain)
{
  ...
  if (FindRoutine (
        &m_ctlMacroList,    // passed back to callback routines
...
void CPrefsP7::DoDataExchange(CDataExchange* pDX)
{
  ...
  if (pDX->m_bSaveAndValidate)
    if (m_ctlUseDefaultAliases.GetCheck () &&
        m_ctlUseDefaultAliases.GetCheck () != m_bUseDefaultAliases &&
        m_doc->m_AliasMap.GetCount () > 0 &&
        ::UMessageBox (
          TFormat (
              "By checking the option \"Override with default aliases\" "
              " your existing %i aliase(s) will be PERMANENTLY discarded next time you open this world.\n\n"
              "Are you SURE you want to do this?",
            m_doc->m_AliasMap.GetCount ()
            ),
          MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2
          )
        != IDYES)
...
CObject * CPrefsP7::MakeNewObject ()
{
...
int CPrefsP7::CompareObjects (const int iColumn, const CObject * item1, const CObject * item2)
{
  ...
  switch (iColumn)   // which sort key
    ...
    case eColumnSequence:
      ...
      break;
    case eColumnGroup: 
      iResult = alias1->strGroup.CompareNoCase (alias2->strGroup);
      if (iResult)
        break;
    case eColumnLabel: 
      iResult = alias1->strLabel.CompareNoCase (alias2->strLabel); 
      if (iResult)
        break;
    case eColumnAlias:
      iResult = alias1->name.CompareNoCase (alias2->name); 
      break;
    case eColumnContents:
      iResult = alias1->contents.CompareNoCase (alias2->contents);
      break;
    default:
      iResult = 0;
    } // end of switch
                       ...
                       const BOOL insert)
{
  ...
  m_ctlAliasList.SetItemText (
      nItem, eColumnContents,
...
void CPrefsP7::OnMoveUp() 
{
  ...
  for (int nItem = -1;
    ...
    if (alias->iSequence == 0)
      ...
      if (bSame)
        alias->iSequence -= 1;   // make a lower sequence (drop out of group)
      else if (iLower == -1)
  ...
  // redraw the list
  m_ctlList->RedrawItems (0, m_ctlList->GetItemCount () - 1);
}
...
void CPrefsP7::OnMoveDown() 
{
  ...
  for (int nItem = -1;
    ...
    if (alias->iSequence == 10000)
      ...
      if (bSame)
        alias->iSequence += 1;   // make a higher sequence (jump out of group)
      else if (iHigher == -1)
...
{
  pCmdUI->Enable (
      m_last_col == eColumnSequence && !m_reverse &&
      m_ctlList->GetSelectedCount () == 1 &&
      (m_ctlUseDefaultAliases.GetCheck () == 0 ||
       App.m_strDefaultAliasesFile.IsEmpty ())
      );
...
CPrefsP8::CPrefsP8()
  : CGenPropertyPage(CPrefsP8::IDD),
    m_prefsp3(NULL), m_prefsp14(NULL)
{
...
void CPrefsP8::DoDataExchange(CDataExchange* pDX)
{
  ...
  if (pDX->m_bSaveAndValidate)
    if (m_ctlUseDefaultTriggers.GetCheck () &&
        m_ctlUseDefaultTriggers.GetCheck () != m_bUseDefaultTriggers &&
        m_doc->m_TriggerMap.GetCount () > 0 &&
        ::UMessageBox (
          TFormat (
              "By checking the option \"Override with default triggers\" "
              " your existing %i trigger%s will be PERMANENTLY discarded next time you open this world.\n\n"
              "Are you SURE you want to do this?",
            PLURAL (m_doc->m_TriggerMap.GetCount ())
            ),
          MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2
          )
        != IDYES)
...
void CPrefsP8::UnloadDialog (CDialog * pDlg, CObject * pItem)
{
  ...
  if (dlg->m_bBold)      trigger_item->iStyle |= HILITE;
  if (dlg->m_bItalic)    trigger_item->iStyle |= BLINK;
  if (dlg->m_bUnderline) trigger_item->iStyle |= UNDERLINE;
...
CObject * CPrefsP8::MakeNewObject ()
{
...
int CPrefsP8::CompareObjects (const int iColumn, const CObject * item1, const CObject * item2)
{
  ...
  switch (iColumn)   // which sort key
    ...
    case eColumnSequence:
      ...
      break;
    case eColumnGroup:
      iResult = trigger1->strGroup.CompareNoCase (trigger2->strGroup); 
      if (iResult)
        break;
    case eColumnLabel:
      iResult = trigger1->strLabel.CompareNoCase (trigger2->strLabel); 
      if (iResult)
        break;
    case eColumnTrigger:
      iResult = trigger1->trigger.CompareNoCase (trigger2->trigger); 
      if (iResult)
        break;
    case eColumnContents:
      iResult = trigger1->contents.CompareNoCase (trigger2->contents); 
      break;
    default:
      iResult = 0;
    } // end of switch
...
void CPrefsP8::OnMoveUp() 
{
  ...
  for (int nItem = -1;
    ...
    if (trigger->iSequence == 0)
      ...
      if (bSame)
        trigger->iSequence -= 1;   // make a lower sequence (drop out of group)
      else if (iLower == -1)
        ::TMessageBox ("Cannot move up - already at top of list");
      else
...
void CPrefsP8::OnMoveDown() 
{
  ...
  for (int nItem = -1;
    ...
    if (trigger->iSequence == 10000)
      ...
      if (bSame)
        trigger->iSequence += 1; // make a higher sequence (jump out of group)
      else if (iHigher == -1)
        ::TMessageBox ("Cannot move down - already at bottom of list");
      else
...
bool CPrefsP8::GetFilterFlag ()
{
  return m_ctlFilter.GetCheck ();
}

// get the filter script
CString CPrefsP8::GetFilterScript ()
{
...
void CPrefsP9::OnInputFont() 
{
  ...
  lf.lfCharSet = (BYTE) m_input_font_charset;
  ...
  if (m_input_font_weight == FW_BOLD) strStyle += " bold";
  if (m_input_font_italic)            strStyle += " italic";
...
{
  pCmdUI->Enable (
      m_ctlUseDefaultInputFont.GetCheck () == 0 ||
...
CPrefsP11::CPrefsP11()
  : CPropertyPage(CPrefsP11::IDD),
    m_page_number(1), m_doc(NULL), m_pNotesFindInfo(NULL)
{
...
void CPrefsP11::OnLoadNotes() 
{
  ...
  try
    ...
    DWORD length = (DWORD) f->GetLength ();
    if (length > 32000)
      ::TMessageBox (
...
void CPrefsP11::OnSaveNotes() 
{
  ...
  CString suggested_name = m_doc->m_mush_name;
...
void CPrefsP11::DoFind (bool bAgain)
{
  ...
  while (FindInfo.m_nCurrentLine >= 0 && 
    ...
    if (strText.Mid (FindInfo.m_nCurrentLine, iFindLength) == strFindString)
      break;

    // keep track of line count
    FindInfo.m_nCurrentLine += (FindInfo.m_bForwards) ? 1 : -1;
  ...
  pEdit->SetSel (
      FindInfo.m_nCurrentLine,
      FindInfo.m_nCurrentLine + strFindString.GetLength ()
      );
...
CPrefsP12::CPrefsP12()
  : CPropertyPage(CPrefsP12::IDD),
    m_page_number(1), m_doc(NULL), m_bFirstTime(true)
{
  ...
  if (pDX->m_bSaveAndValidate)
    ...
    if (m_nOldLines != m_nLines && m_nLines > 1000)
      ...
      if (nBytesNeeded > MemStat.dwTotalPhys)
        {
        CString strMsg = TFormat ("You are allocating %ld lines for your output buffer, but have "
...
void CPrefsP14::OnChooseFont() 
{
  ...
  lf.lfCharSet = (BYTE) m_font_charset;
  lf.lfItalic = FALSE;

  CFontDialog dlg (&lf, CF_SCREENFONTS, NULL, GetOwner ()); // owner view
...
void CPrefsP14::OnAdjustWidth() 
{
  ...
  font.CreateFont(
      ...
      0, // BYTE bUnderline, 
      0, // BYTE cStrikeOut, 
      (BYTE) m_font_charset, // BYTE nCharSet, 
...
{
  pCmdUI->Enable (
      m_ctlUseDefaultOutputFont.GetCheck () == 0 ||
...
CPrefsP16::CPrefsP16()
  : CGenPropertyPage(CPrefsP16::IDD)
{
...
void CPrefsP16::DoDataExchange(CDataExchange* pDX)
{
  ...
  if (pDX->m_bSaveAndValidate)
    if (m_ctlUseDefaultTimers.GetCheck () &&
        m_ctlUseDefaultTimers.GetCheck () != m_bUseDefaultTimers &&
        m_doc->m_TimerMap.GetCount () > 0 &&
        ::UMessageBox (
          TFormat (
              "By checking the option \"Override with default timers\" "
              " your existing %i timer%s will be PERMANENTLY discarded next time you open this world.\n\n"
              "Are you SURE you want to do this?",
            PLURAL (m_doc->m_TimerMap.GetCount ())
            ),
          MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2
          ) != IDYES)
      pDX->Fail ();
...
CObject * CPrefsP16::MakeNewObject ()
{
  ...
  switch (iColumn)   // which sort key
    ...
    case eColumnType:
        if (timer1->iType < timer2->iType)
          iResult = -1;
        else if (timer1->iType > timer2->iType)
          iResult = 1;
        break;

    case eColumnWhen:
      {
      CmcDateTimeSpan ts1, ts2;
    ...
    case eColumnGroup:
      iResult = timer1->strGroup.CompareNoCase (timer2->strGroup); 
      if (iResult)
        break;

    case eColumnLabel:
      iResult = timer1->strLabel.CompareNoCase (timer2->strLabel);
      break;

    case eColumnNext:    
      if (timer1->tFireTime < timer2->tFireTime)
        iResult = -1;
      else if (timer1->tFireTime > timer2->tFireTime)
        iResult = 1;
      break;
                        ...
                        const BOOL insert)
{
  ...
  switch (timer_item->iType)
    ...
    case CTimer::eInterval: 
      ...
      if (timer_item->iOffsetHour || 
        ...
        strWhen.Format (
            "%02i:%02i:%04.2f offset %02i:%02i:%04.2f",
  ...
  if (tDue < tNow)
    strDuration = "-";
  else if (floor (ts.GetTotalDays ()) > 0)
    strDuration = CFormat ("%.f d", floor (ts.GetTotalDays ()));
  else if (floor (ts.GetTotalHours ()) > 0)
    strDuration = CFormat ("%.f h", floor (ts.GetTotalHours ()));
  else if (floor (ts.GetTotalMinutes ()) > 0)
...
// is filtering enabled?
bool CPrefsP16::GetFilterFlag ()
{
  return m_ctlFilter.GetCheck ();
}

// get the filter script
CString CPrefsP16::GetFilterScript ()
{
...
{
  CString strSubscript, strPrompt;
  ...
  CString strFilename = (bLoad) ? m_strScriptFilename : "";
...
void CPrefsP17::OnNewScript() 
{
  ...
  try
    {
    CFile (m_strScriptFilename, CFile::modeCreate | CFile::modeReadWrite);
    } // end of try block
  catch (CException * e)
    {
    e->ReportError ();
...
BOOL CPrefsP17::OnInitDialog() 
{
  ...
  else if (m_strLanguage.CompareNoCase ("jscript") == 0)
    nItem = SCRIPT_JSCRIPT;
  else if (m_strLanguage.CompareNoCase ("perlscript") == 0)
    nItem = SCRIPT_PERL;
  else if (m_strLanguage.CompareNoCase ("python") == 0)
    nItem = SCRIPT_PYTHON;
  else if (m_strLanguage.CompareNoCase ("tclscript") == 0)
    nItem = SCRIPT_TCL;
  else if (m_strLanguage.CompareNoCase ("lua") == 0)
    nItem = SCRIPT_LUA;
  else if (m_strLanguage.CompareNoCase ("phpscript") == 0)
    nItem = SCRIPT_PHP;
  else if (m_strLanguage.CompareNoCase ("rubyscript") == 0)
    nItem = SCRIPT_RUBY;
...
void CPrefsP17::OnRegister() 
{
  ...
  try
    ...
    HRESULT (FAR STDAPICALLTYPE * lpDllEntryPoint)(void); // function pointer
    ...
    CHECKHRESULT_EXCEPTION hr = (*lpDllEntryPoint) ();

   ::UMessageBox (
       TFormat ("Successfully registered %s", (LPCTSTR) strDLLname),
       MB_ICONINFORMATION
       );
    } // end of try block
  catch (CException * e)
...
CPrefsP18::CPrefsP18()
  : CGenPropertyPage(CPrefsP18::IDD)
{
...
CObject * CPrefsP18::MakeNewObject ()
{
...
int CPrefsP18::CompareObjects (const int iColumn, const CObject * item1, const CObject * item2)
{
  ...
  switch (iColumn)   // which sort key
    {
    case eColumnName:
      iResult = variable1->strLabel.CompareNoCase (variable2->strLabel);
      break;
    case eColumnContents:
      iResult = variable1->strContents.CompareNoCase (variable2->strContents);
      break;
    default:
      iResult = 0;
      break;
...
void CPrefsP18::OnLoadVariables() 
{
  ...
  if (iBadLines > 0)
    ::UMessageBox (
        TFormat ("%i line%s could not be added as a variable.", PLURAL (iBadLines)),
        MB_ICONEXCLAMATION
        );
  ...
  ::UMessageBox (
      TFormat ("Loaded %i variable%s.", PLURAL (iCount)),
      MB_ICONINFORMATION
      );
}
...
void CPrefsP18::OnSaveVariables() 
{
  ...
  try
    ...
    f = new CFile (dlg.GetPathName (), CFile::modeCreate | CFile::modeReadWrite);
  ...
  ::UMessageBox (
      TFormat ("Saved %i variable%s.", PLURAL (iCount)),
      MB_ICONINFORMATION
      );
}
...
// is filtering enabled?
bool CPrefsP18::GetFilterFlag ()
{
  return m_ctlFilter.GetCheck ();
}

// get the filter script
CString CPrefsP18::GetFilterScript ()
{
...
void CPrefsP19::DoDataExchange(CDataExchange* pDX)
{
  ...
  if (pDX->m_bSaveAndValidate)
    ...
    if (m_bEnableAutoSay && // test is only relevant if enabled
        m_strAutoSayString.IsEmpty ())
      {
      ::TMessageBox("Your \"auto say\" string cannot be blank");
      DDX_Text(pDX, IDC_AUTO_SAY_STRING, m_strAutoSayString);
      pDX->Fail();
      } // end of auto say enabled and string being blank
    } // end of saving and validating
}
...
void CPrefsP21::OnUpdateLineCount(CCmdUI* pCmdUI)
{
  ...
  if (!strText.IsEmpty ())
    {
    // count lines
    for (const char * p = strText; *p != '\0'; ++p)
      if (*p == '\n')
        iCount += 1;
...
void CPrefsP14::OnAdjustToWidth() 
{
  ...
  int iColumn = atoi (GetText (m_ctlWrapColumn));
  if (iColumn < 20)
    iColumn = 20;
  else if (iColumn > MAX_LINE_WIDTH)
