...
CTriggerDlg::CTriggerDlg(CWnd* pParent /*=NULL*/)
  : CDialog(CTriggerDlg::IDD, pParent), m_pDoc(NULL),
    m_pTriggerMap(NULL), m_current_trigger(NULL), m_bUTF_8(false),
    m_font(NULL), m_font2(NULL)
{
...
void CTriggerDlg::DoDataExchange(CDataExchange* pDX)
{
  ...
  if (pDX->m_bSaveAndValidate)
    ...
    if (m_bRegexp &&
        !CheckRegularExpression (m_trigger,
    ...
    for (POSITION pos = m_pTriggerMap->GetStartPosition (); pos;)
      {
      ...
      if (!m_strLabel.IsEmpty () && // we can have duplicate blank names
          m_strLabel.CompareNoCase (trigger_item->strLabel) == 0)
        {
        ::UMessageBox(
            TFormat (
    ...
    else if (CheckLabel (m_strVariable))
      {
      ::TMessageBox(
            "The variable must start with a letter and consist of letters, "
    ...
    // check procedure is valid
      ...
      {
      ::TMessageBox(
            "The script subroutine name must start with a letter and consist of letters, "
            "numbers or the underscore character.",
          MB_ICONSTOP
          );
      DDX_Text(pDX, IDC_SCRIPT_NAME, m_strProcedure);
      pDX->Fail();
      } // end of checking valid procedure


    // warn if contents empty
    if (m_iSendTo != eSendToWorld && m_contents.IsEmpty () &&
        ::UMessageBox(TFormat ("Your trigger is set to 'send to %s' however the 'Send:' field is blank.\n\n"
...
BOOL CTriggerDlg::OnInitDialog() 
{
  ...
  m_ctlVariable.EnableWindow (
      m_iSendTo == eSendToVariable 
#ifdef PANE
...
void CTriggerDlg::OnUpdateInvocationCount(CCmdUI* pCmdUI)
{
  ...
  CString strMessage;
  strMessage.Format ("%ld call%s.", PLURAL(m_current_trigger->nInvocationCount));
...
void CTriggerDlg::OnUpdateCheckComboColour(CCmdUI* pCmdUI)
{
  ...
  if (m_ctlTriggerColour.GetCurSel () == OTHER_CUSTOM + 1 &&
      (m_ctlTriggerSwatch.m_colour != m_ctlTriggerColour.m_iOtherForeground ||
...
void CTriggerDlg::OnConvertToRegexp() 
{
  ...
  m_ctlTrigger.SetWindowText (ConvertToRegularExpression (strText));
