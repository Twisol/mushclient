...
{
  return (m_ScriptEngine) ? m_ScriptEngine->GetDispid (sName) : DISPID_UNKNOWN;
}
...
bool CPlugin::SaveState ()
{
  if (!m_bSaveState ||
      App.m_strPluginsDirectory.IsEmpty () || // need a directory
      m_pDoc->m_strWorldID.IsEmpty ()         // need a world id
    )
    return true;
  ...
  try
    ...
    m_pDoc->m_CurrentPlugin = this;
    m_pDoc->Save_World_XML (*ar, XML_VARIABLES,
        TFormat ("Plugin state saved. Plugin: \"%s\". World: \"%s\".",
          (LPCTSTR) m_strName,
...
void CMUSHclientDoc::OnFilePluginwizard() 
{
  ...
  // select everything
  // triggers
  for (POSITION pos = m_TriggerMap.GetStartPosition(); pos;)
    {
    CTrigger * t;
  ...
  // aliases
  for (POSITION pos = m_AliasMap.GetStartPosition(); pos;)
    {
    CAlias * a;
  ...
  // variables
  for (POSITION pos = m_VariableMap.GetStartPosition(); pos;)
    {
    CVariable * v;
  ...
  propSheet.m_Page6.m_bSaveState = m_VariableMap.GetCount() > 0;
  ...
  try
    ...
    if (propSheet.m_Page2.m_bGenerateHelp &&
      ...
      else if (propSheet.m_Page7.m_strLanguage.CompareNoCase ("jscript") == 0)
        {
      ...
      else if (propSheet.m_Page7.m_strLanguage.CompareNoCase ("perlscript") == 0)
        {
      ...
      else if (propSheet.m_Page7.m_strLanguage.CompareNoCase ("python") == 0)
        {
        ar.WriteString ("def OnHelp (sName, sLine, wildcards):" NL);
        ar.WriteString ("  world.Note (world.GetPluginInfo (world.GetPluginID, 3))" NL);
        }
      else if (propSheet.m_Page7.m_strLanguage.CompareNoCase ("tclscript") == 0)
        {
        ar.WriteString ("def OnHelp (sName, sLine, wildcards):" NL);
        ar.WriteString ("  world.Note (world.GetPluginInfo (world.GetPluginID, 3))" NL);
        }
      else if (propSheet.m_Page7.m_strLanguage.CompareNoCase ("lua") == 0)
        {
        ar.WriteString ("function OnHelp ()" NL);
        ar.WriteString ("  world.Note (world.GetPluginInfo (world.GetPluginID (), 3))" NL);
        ar.WriteString ("end" NL);        
        }
      else if (propSheet.m_Page7.m_strLanguage.CompareNoCase ("phpscript") == 0)
        {
        ar.WriteString ("function OnHelp ()" NL);
        ar.WriteString ("  {" NL);        
        ar.WriteString ("  world.Note (world.GetPluginInfo (world.GetPluginID (), 3));" NL);
      ...
      else if (propSheet.m_Page7.m_strLanguage.CompareNoCase ("rubyscript") == 0)
        {
