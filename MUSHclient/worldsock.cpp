...
{
  if (nErrorCode != 0) // had an error, give up!
    return;

  // if we have no outstanding data to send, don't bother
  if (m_outstanding_data.GetLength () <= 0)
    return;

  int count = Send (m_outstanding_data, m_outstanding_data.GetLength ());
...
void CWorldSocket::OnClose(int nErrorCode)
{
  ...
  // CLEANUP: the world socket shouldn't know or care how many view buttons exist
  if (m_pDoc->m_view_number >= 1 && m_pDoc->m_view_number <= 10)
    Frame.OnUpdateBtnWorlds (m_pDoc->m_view_number, NULL);

  // execute "disconnect" script
  if (m_pDoc->m_ScriptEngine && m_pDoc->SeeIfHandlerCanExecute (m_pDoc->m_strWorldDisconnect))
    {
  ...
  if (m_pDoc->m_bShowConnectDisconnect && !bWasClosed)
    ...
    m_pDoc->Note (theTime.Format (TranslateTime ("--- Disconnected on %A, %B %d, %Y, %#I:%M %p ---")));

    // find time spent connected
    CTimeSpan ts = theTime - m_pDoc->m_tConnectTime;
  ...
  m_pDoc->Note (TFormat ("--- Received %i line%s, sent %i line%s.",
                PLURAL (m_pDoc->m_nTotalLinesReceived),
                PLURAL (m_pDoc->m_nTotalLinesSent)
                ));

  m_pDoc->Note (TFormat ("--- Output buffer has %i/%i line%s in it (%.1f%% full).",
  ...
  m_pDoc->Note (TFormat ("--- Matched %i trigger%s, %i alias%s, and %i timer%s fired.",
